...
const CTA = () => {
  ...
  const handleCTASignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCTAForm({ companyName: '', email: '', phone: '', budget: '', password: '' });
            ...
            <DialogContent className="bg-white/95 backdrop-blur-xl border-white/20 shadow-2xl max-w-md rounded-2xl">
              ...
              <form onSubmit={handleCTASignup} className="space-y-6 p-2">
                <div className="space-y-2">
                  ...
                  <Input 
                    id="company-name" 
                    value={ctaForm.companyName}
                    onChange={(e) => setCTAForm({...ctaForm, companyName: e.target.value})}
                ...
                <div className="space-y-2">
                  ...
                  <Input 
                    ...
                    onChange={(e) => setCTAForm({...ctaForm, budget: e.target.value})}
                    placeholder="$10,000" 
                    className="bg-white/80 border-slate-200 text-slate-800 focus:border-blue-400 focus:ring-2 focus:ring-blue-100 rounded-xl h-12 px-4 transition-all duration-200 placeholder:text-slate-400" 
                    required
                  />
                ...
                <div className="space-y-2">
                  ...
                  <Input 
                    id="cta-password" 
                    type="password" 
                    value={ctaForm.password}
                    onChange={(e) => setCTAForm({...ctaForm, password: e.target.value})}
                    className="bg-white/80 border-slate-200 text-slate-800 focus:border-blue-400 focus:ring-2 focus:ring-blue-100 rounded-xl h-12 px-4 transition-all duration-200 placeholder:text-slate-400" 
                ...
                <Button 
                  type="submit" 
                  className="w-full bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white h-12 rounded-xl font-semibold shadow-lg hover:shadow-xl transform hover:scale-[1.02] transition-all duration-200"
                  disabled={isLoading}
                >
